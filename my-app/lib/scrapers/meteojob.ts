...
export async function scrapeMeteojob(skills: string[]): Promise<ScraperResult> {
  ...
  try {
    const res = await axios.get<string>(`${BASE}/jobs?q=${encodeURIComponent(query)}`, {
      headers: {
        'User-Agent':
    ...
    if (!stateText) {
      return { jobs: [], source: 'meteojob', error: 'Meteojob: état Angular introuvable' };
    }
